...
class ParagraphOracle:
    ...
    def consume(self, text: str) -> tuple[list[dict], str, int, str, list[dict]]:
        ...
        for index, consumed_entry in enumerate(consumed_entries):
            ...
            segments.append({"start": start, "end": end, "a": address, "c": context, "d": depth, "u": entry_units, "q": bool(entry["term"] or entry["disabled"])})
...
def generate(cache: pathlib.Path, output: pathlib.Path) -> dict:
    ...
    # The browser updater calls this same JavaScript engine after normalization.
    import subprocess
    reconciled = subprocess.run(
        ["node", str(ROOT / "tools" / "reconcile-cfr.js"), "--report", str(ROOT / "sources/legal/cfr-hierarchy/corrections.json")],
        input=json.dumps(result, ensure_ascii=False), text=True, capture_output=True,
    )
    if reconciled.returncode:
        raise ValueError(f"CFR hierarchy reconciliation failed: {reconciled.stderr.strip()}")
